...
def find_centroid(cluster):
    ...
    # BEGIN Question
    #return the centroid of the locations obtained from the mean of latitude and langitude of each restaurant from cluster
    return [mean([l[0] for l in [restaurant_location(c) for c in cluster]]), 
    mean([l[1] for l in [restaurant_location(c) for c in cluster]])]
    # END Question 5
...
def best_predictor(user, restaurants, feature_fns):
    ...
    for fn in feature_fns:
        ...
        set_r_squared.append(r_squared)
    highest = set_r_squared.index(max(set_r_squared))
    return predictors[highest]
    # END Question 8
